...
from pyindexnum.multilateral import (
    geks_fisher, geks_tornqvist,
    geary_khamis,
    time_product_dummy,
)

BILATERAL_FUNCS = {
    "Jevons": jevons, "Carli": carli, "Dudot": dudot,
    "Laspeyres": laspeyres, "Paasche": paasche, "Fisher": fisher,
    "Tornqvist": tornqvist, "Walsh": walsh,
}
...
AGG_OPTIONS_WEIGHTED = ["weighted_arithmetic", "weighted_geometric", "weighted_harmonic"]
...
class PyIndexApp:
    ...
    def _show_snack(self, message: str, color=ft.Colors.BLUE):
        self.page.show_dialog(
            ft.SnackBar(content=ft.Text(message, color=color))
        )
    ...
    async def _handle_tab_change(self, e):
        ...
        elif idx >= 4 and (self.bilateral_result is None and self.multilateral_result is None):
    ...
    def _build_import_tab(self):
        self._load_btn = ft.Button("Load Data", icon=ft.Icons.UPLOAD_FILE, on_click=self._handle_load)
        ...
        self._map_dd = {}
        for name, req in [("date", True), ("price", True), ("product_id", True), ("quantity", False)]:
            dd = ft.Dropdown(
        ...
        self._std_btn = ft.Button(
            "Standardize Columns", icon=ft.Icons.CHECK, on_click=self._handle_standardize, disabled=True,
        )
        ...
        self._import_content = ft.Container(
            padding=20,
            content=ft.Column(
                [self._load_btn, ft.Divider(), self._raw_preview, ft.Divider(),
                 self._mapping_section, ft.Divider(), self._import_summary],
    ...
    async def _handle_standardize(self, e):
        ...
        for name, dd in self._map_dd.items():
            ...
            if val and val != "None":
                ...
                self._show_snack(f"Required field '{name}' is not mapped", ft.Colors.RED)
                return
    ...
    def _build_aggregation_tab(self):
        self._freq_dd = ft.Dropdown(
            label="Frequency", width=200, value="1mo",
            options=[ft.dropdown.Option(f) for f in FREQ_OPTIONS],
        )
        self._agg_type_dd = ft.Dropdown(
            label="Aggregation type", width=250, value="arithmetic",
            options=[ft.dropdown.Option(a) for a in AGG_OPTIONS_UNWEIGHTED],
        )
        self._agg_btn = ft.Button("Run Aggregation", icon=ft.Icons.PLAY_ARROW, on_click=self._handle_aggregate)
    ...
    async def _handle_aggregate(self, e):
        ...
        if agg_type.startswith("weighted_") and not self.has_quantity:
            self._show_snack("Weighted aggregation requires quantity data", ft.Colors.RED)
            return

        try:
            self.aggregated_df = aggregate_time(
                self.standardized_df,
                date_col="date", price_col="price", id_col="product_id",
                quantity_col="quantity" if self.has_quantity else None,
                agg_type=agg_type, freq=freq,
            )
        except Exception as err:
    ...
    def _build_balancing_tab(self):
        ...
        self._remove_unbal_btn = ft.Button(
            "Remove Unbalanced Products", icon=ft.Icons.FILTER_ALT, on_click=self._handle_remove_unbalanced,
        )
        self._imputation_dd = ft.Dropdown(
            label="Imputation method", width=250, value="none",
            options=[
        ...
        self._impute_btn = ft.Button(
            "Apply Imputation", icon=ft.Icons.AUTO_FIX_HIGH, on_click=self._handle_imputation,
        )
    ...
    async def _handle_imputation(self, e):
        ...
        try:
            if method == "Carry Forward":
                self.balanced_df = carry_forward_imputation(self.balanced_df, value_cols)
            else:
                self.balanced_df = carry_backward_imputation(self.balanced_df, value_cols)
        except Exception as err:
    ...
    def _build_index_tab(self):
        ...
        self._ml_btn = ft.Button("Calculate", icon=ft.Icons.PLAY_ARROW, on_click=self._handle_multilateral)
        ...
        self._idx_content = ft.Container(
            ...
            content=ft.Column(
                [
                    ...
                    ft.Row([self._bi_base_dd, self._bi_comp_dd, self._bi_method_dd, self._bi_btn]),
                    self._bi_result,
                    ft.Divider(),
    ...
    async def _handle_bilateral(self, e):
        ...
        if not base or not comp or not method:
            self._show_snack("Select base, comparison period, and method", ft.Colors.RED)
            return
        if base == comp:
        ...
        try:
            base_dt = pl.Series("period", [base, comp]).str.strptime(pl.Date, "%Y-%m-%d")
        ...
        self._bi_result.value = f"{method} Index ({base} → {comp}): {self.bilateral_result:.6f}"
    ...
    async def _handle_multilateral(self, e):
        ...
        try:
            ...
            if method == "Geary-Khamis":
                ...
                self.multilateral_result = func(
                    self.balanced_df, weighted=self._ml_weighted_sw.value,
                )
            else:
    ...
    def _refresh_export_tab(self):
        ...
        if self.bilateral_result is not None:
            ...
            controls += [
                ...
                ft.Text(f"{info['method']}: {self.bilateral_result:.6f}  ({info['base']} → {info['comp']})"),
                ft.Row([
                    ft.Button("Export CSV", icon=ft.Icons.DOWNLOAD,
                              data=("bilateral", "csv"), on_click=self._handle_export),
                    ft.Button("Export Excel", icon=ft.Icons.DOWNLOAD,
                              data=("bilateral", "xlsx"), on_click=self._handle_export),
                ]),
                ft.Divider(),
            ]
        if self.multilateral_result is not None:
            controls += [
                ...
                ft.Row([
                    ft.Button("Export CSV", icon=ft.Icons.DOWNLOAD,
                              data=("multilateral", "csv"), on_click=self._handle_export),
                    ft.Button("Export Excel", icon=ft.Icons.DOWNLOAD,
                              data=("multilateral", "xlsx"), on_click=self._handle_export),
                ]),
                ft.Divider(),
            ]
        if self.balanced_df is not None:
            controls += [
                ...
                ft.Row([
                    ft.Button("Export CSV", icon=ft.Icons.DOWNLOAD,
                              data=("balanced", "csv"), on_click=self._handle_export),
                    ft.Button("Export Excel", icon=ft.Icons.DOWNLOAD,
                              data=("balanced", "xlsx"), on_click=self._handle_export),
                ]),
            ]
    ...
    async def _handle_export(self, e):
        ...
        result = await ft.FilePicker().save_file(
            allowed_extensions=[ext], file_name=f"{target}_data.{ext}",
        )
